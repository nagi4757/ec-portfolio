...
The dev UI is at `http://localhost:5173` (Vite binds to `127.0.0.1`, with a strict port).
...
The login response's `user.role` is used only for the ADMIN navigation guard. No JWT payload is decoded. Browser storage is not trusted for authorization: the backend remains the source of truth and enforces every Admin API request.
...
Build runs `tsc -b` before Vite. It checks TypeScript and creates `apps/admin-web/dist`; it does not verify network connectivity or server behavior.

## Live integration checklist

- Admin login: ADMIN enters; USER receives an administrator-account message without saving that login.
- Product/category lists: loading, empty, load failure/retry, and forbidden responses.
- Product/category forms: create/update, missing record, invalid route ID, and failed saves retaining input.
- Product deactivation/reactivation and category deletion: 204 responses, failures, and refresh.
- Order list/detail/status transitions: permitted transitions, 404/409 feedback, and shipping details.
